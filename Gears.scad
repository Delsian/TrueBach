...
function GearsSourceRod() = 5;

module GearsMiddleSet()
{
...
module GearsMiddleWorm() {
	difference() {
		...
		translate ([0,0,-.5]) cylinder(h=length*1.1, d=GearsSourceRod()*1.1);
		translate ([0,0,-.5]) hex_nut(GearsSourceRod());
	}
}

module GearsMiddleGear() {
